...
import type {
  BionProductCard, BionCartDrawer, BionQuantityStepper,
  BionChip, BionAccountBadge,
...
function useBridge<T extends HTMLElement>(
  ...
  useEffect(() => {
    ...
    return () => handlers.forEach(([type, h]) => el.removeEventListener(type, h));
...
export interface ProductCardProps {
  name: string; brand?: string; price: string; oldPrice?: string;
  glyph?: string; image?: string; tag?: string; rating?: number;
  reviews?: string; favorite?: boolean;
...
export function ProductCard(p: ProductCardProps) {
  ...
  useBridge(ref, { rating: p.rating, favorite: p.favorite },
    { 'bion-add': p.onAdd ?? (() => {}), 'bion-favorite': p.onFavorite ?? (() => {}), 'bion-select': p.onSelect ?? (() => {}) },
    [p.rating, p.favorite], [p.onAdd, p.onFavorite, p.onSelect]);
  return h('bion-product-card', {
    ref, name: p.name, brand: p.brand, price: p.price, 'old-price': p.oldPrice,
    glyph: p.glyph, image: p.image, tag: p.tag, reviews: p.reviews,
    class: p.className, style: p.style,
  });
}

/* ── Button ── */
export interface ButtonProps {
  variant?: 'solid' | 'outline' | 'icon'; icon?: string; label?: string;
  disabled?: boolean; onClick?: () => void; children?: React.ReactNode;
  className?: string; style?: React.CSSProperties;
}
export function Button(p: ButtonProps) {
  return h('bion-button', {
    variant: p.variant, icon: p.icon, label: p.label, disabled: p.disabled || undefined,
    onClick: p.onClick, class: p.className, style: p.style,
  }, p.children);
}

/* ── Chip ── */
export interface ChipProps {
  value: string; active?: boolean; onSelect?: (d: { value: string }) => void;
  children?: React.ReactNode;
}
export function Chip(p: ChipProps) {
  const ref = useRef<BionChip>(null);
  useBridge(ref, { active: p.active }, { 'bion-select': p.onSelect ?? (() => {}) }, [p.active], [p.onSelect]);
  return h('bion-chip', { ref, value: p.value }, p.children);
}

/* ── Rating ── */
export interface RatingProps { value: number; reviews?: string; max?: number; }
...
/* ── Icon ── */
export interface IconProps { name: string; size?: number; strokeWidth?: number; }
export function Icon(p: IconProps) {
  return h('bion-icon', { name: p.name, size: p.size, 'stroke-width': p.strokeWidth });
}
...
export function QuantityStepper(p: QuantityStepperProps) {
  ...
  useBridge(ref, { value: p.value, min: p.min, max: p.max },
    { 'bion-change': p.onChange ?? (() => {}) }, [p.value, p.min, p.max], [p.onChange]);
  return h('bion-quantity-stepper', { ref });
}

/* ── AccountBadge ── */
export interface AccountBadgeProps {
  name: string; initials?: string; onSelect?: (d: { name: string }) => void;
}
export function AccountBadge(p: AccountBadgeProps) {
  const ref = useRef<BionAccountBadge>(null);
  useBridge(ref, {}, { 'bion-select': p.onSelect ?? (() => {}) }, [], [p.onSelect]);
  return h('bion-account-badge', { ref, name: p.name, initials: p.initials });
}

/* ── Carousel ── */
export interface CarouselProps {
  aspect?: string; interval?: number; children?: React.ReactNode;
}
export function Carousel(p: CarouselProps) {
  return h('bion-carousel', { aspect: p.aspect, interval: p.interval }, p.children);
}

/* ── CartDrawer ── */
export interface CartDrawerProps {
  items: CartLine[]; subtotal: string; total: string; shipping?: string; open?: boolean;
  onClose?: () => void;
...
export function CartDrawer(p: CartDrawerProps) {
  ...
  useBridge(ref, { items: p.items, open: p.open },
    {
      'bion-close': () => p.onClose?.(),
      'bion-quantity-change': (d) => p.onQuantityChange?.(d),
      'bion-checkout': () => p.onCheckout?.(),
    },
    [p.items, p.open], [p.onClose, p.onQuantityChange, p.onCheckout]);
  return h('bion-cart-drawer', { ref, subtotal: p.subtotal, total: p.total, shipping: p.shipping });
}
...
declare global {
  namespace JSX {
    interface IntrinsicElements {
      [el: `bion-${string}`]: React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> & Record<string, unknown>;
    }
  }
}
