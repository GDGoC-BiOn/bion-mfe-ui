...
export class BionButton extends LitElement {
  ...
  static styles = [
    ...
    css`
      ...
      .solid { background: var(--bion-color-ink); color: var(--bion-color-on-ink); padding: 13px 24px; }
      .solid:hover:not(:disabled) { gap: var(--bion-space-3); }
      .outline { background: none; color: var(--bion-color-ink); border-color: var(--bion-color-line2); padding: 9px 16px; }
      .outline:hover:not(:disabled) { border-color: var(--bion-color-ink); }
      .icon { width: 40px; height: 40px; padding: 0; border-radius: 50%; background: var(--bion-color-ink); color: var(--bion-color-on-ink); }
  ...
  render() {
    ...
    const glyph = this.icon
      ? html`<svg viewBox="0 0 24 24" width=${iconSize} height=${iconSize} fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${unsafeSVG(icons[this.icon])}</svg>`
      : nothing;
...
declare global {
  interface HTMLElementTagNameMap { 'bion-button': BionButton; }
}
