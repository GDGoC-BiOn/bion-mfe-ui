...
export class BionChip extends LitElement {
  ...
  static styles = [
    ...
    css`
      ...
      button:hover { color: var(--bion-color-ink); }
      :host([active]) button { color: var(--bion-color-ink); font-weight: var(--bion-font-weight-medium); }
      :host([active]) button::after {
        content: ''; position: absolute; left: 0; right: 0; bottom: -6px;
        height: 1px; background: var(--bion-color-ink);
      }
    `,
  ];
  ...
  private onClick() {
    this.dispatchEvent(
      new CustomEvent('bion-select', { detail: { value: this.value }, bubbles: true, composed: true })
    );
  }

  render() {
    return html`<button aria-pressed=${this.active} @click=${this.onClick}><slot></slot></button>`;
  }
}

declare global {
  interface HTMLElementTagNameMap { 'bion-chip': BionChip; }
}
