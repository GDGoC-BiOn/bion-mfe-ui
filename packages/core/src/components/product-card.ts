...
export class BionProductCard extends LitElement {
  ...
  static styles = [
    ...
    css`
      ...
      .add svg {
        transition: transform var(--bion-motion-base) var(--bion-motion-ease);
      }
      .add:hover svg {
      ...
      .price s {
        ...
        font-size: var(--bion-font-size-xs);
        margin-left: 2px;
      }
    `,
  ];
  ...
  render() {
    return html`
      <article
        class="card"
        @click=${() => this.emit("bion-select", { name: this.name })}
      >
        <div class="thumb">
          ...
          ${this.image
            ...
            : html`<svg
                ...
                stroke-width=".9"
              >
                ${unsafeSVG(icons[this.glyph])}
              </svg>`}
          <button
            class="add"
            aria-label=${`Add ${this.name} to cart`}
            @click=${this.onAdd}
          >
            <svg
              viewBox="0 0 24 24"
        ...
        <div class="body">
          ${this.brand
            ? html`<span class="brand">${this.brand}</span>`
            : nothing}
          <span class="name">${this.name}</span>
          ${this.rating
            ? html`<span class="rating"
                ><span class="stars">${this.renderStars()}</span>${this.reviews
                  ? html`<span class="reviews">(${this.reviews})</span>`
                  : nothing}</span
              >`
            : nothing}
          <span class="price"
            >${this.price}${this.oldPrice
              ? html`<s>${this.oldPrice}</s>`
              : nothing}</span
          >
        </div>
      </article>
