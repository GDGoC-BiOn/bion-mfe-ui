...
export class BionRating extends LitElement {
  ...
  static styles = [
    hostTokens,
    css`
      :host { display: inline-flex; align-items: center; gap: var(--bion-space-1); }
      ...
      .count { font-size: var(--bion-font-size-2xs); color: var(--bion-color-mute); }
    `,
  ];
  ...
  render() {
    ...
    return html`
      <span class="stars" role="img" aria-label=${`${this.value} of ${this.max} stars`}>${stars}</span>
      ${this.reviews ? html`<span class="count">(${this.reviews})</span>` : nothing}
    `;
  }
}

declare global {
  interface HTMLElementTagNameMap { 'bion-rating': BionRating; }
}
