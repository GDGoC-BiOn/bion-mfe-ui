...
export class BionAccountBadge extends LitElement {
  ...
  static styles = [
    ...
    css`
      ...
      button {
        display: flex; align-items: center; gap: var(--bion-space-2);
        background: none; border: none; cursor: pointer; padding: 0;
        font-family: var(--bion-font-body); font-size: var(--bion-font-size-sm);
      ...
      .av {
        ...
        font-family: var(--bion-font-display); font-weight: var(--bion-font-weight-medium);
        font-size: var(--bion-font-size-2xs); letter-spacing: .02em;
      }
    `,
  ];
  ...
  private onClick() {
    this.dispatchEvent(new CustomEvent('bion-select', { detail: { name: this.name }, bubbles: true, composed: true }));
  }
...
declare global {
  interface HTMLElementTagNameMap { 'bion-account-badge': BionAccountBadge; }
}
